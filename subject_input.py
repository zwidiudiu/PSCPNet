...
def random_swap(train_batch, label_image_list, channel_list):
...
def random_channel_swap(train_batch, train_labels):
    # 功能：随机做通道交换, 针对的是时序图
    ...
    random_swap(train_batch, label_is0_image_list, channel_list)
    random_swap(train_batch, label_is1_image_list, channel_list)
    return train_batch # shape=[batch_size, channels, samples]

# 生成训练批次
def return_train_set(file_path, batch_size, window_size, label_index, method, fold, sub):
    ...
    total_data = total_data[sub, :, :, :] # 暂用 1人测试-[video, channels, samples]
    # 10折交叉验证
    ...
    elif fold == 5: train_data = np.delete(total_data, [i for i in range(128 * 6, 128 * 24)], axis=2)
    elif fold == 6: train_data = np.delete(total_data, [i for i in range(128 * 12, 128 * 30)], axis=2)
    elif fold == 7: train_data = np.delete(total_data, [i for i in range(128 * 18, 128 * 36)], axis=2)
    elif fold == 8: train_data = np.delete(total_data, [i for i in range(128 * 24, 128 * 42)], axis=2)
    elif fold == 9: train_data = np.delete(total_data, [i for i in range(128 * 30, 128 * 48)], axis=2)
    elif fold == 10: train_data = np.delete(total_data, [i for i in range(128 * 36, 128 * 54)], axis=2)
    frame_group = []
    labels_group = []
    while(True):
...
# 生成测试批次
def return_test_set(file_path, batch_size, window_size, label_index, method, fold, sub):
    # label_index: 标签的下标，选择分类类别，与测试集保持严格相同
    # label_index: [0,1,2,3]-[valence,arousal,dominance,liking]
    total_data = np.load(file_path + "\\data_%d.npy" % method) # shape=[32, 40, 32, 7680]
    total_labels = np.load(file_path + "\\labels.npy") # shape=[32, 40, 4]
    ...
    elif fold == 2: test_data = np.delete(total_data, [i for i in range(128 * 6, 128 * 48)], axis=2)
    elif fold == 3: test_data = np.delete(total_data, [i for i in range(128 * 12, 128 * 54)], axis=2)
    ...
    while(True):
        #for people in range(32):
            for video in range(40):
                for i in range(18):
                    segment = test_data[video, :, i*window_size:(i+1)*window_size] #(32, window_size)
