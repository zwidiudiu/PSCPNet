...
reshaped1 = tf.reshape(conv1_4, [-1, flattened_shape1], name="reshaped1") # 空间卷积
...
# 全连接层1
fc1_1 = fc_op(reshaped, name="fc1_1", num_out=512, regularize=regularize_l2)
# 全连接层2
...
acc = np.array(fold_list).reshape([10, epoch_num])  # shape=[10,epoch_num]
loss_end = np.array(fold_loss).reshape([10, epoch_num])  # shape=[10,epoch_num]
